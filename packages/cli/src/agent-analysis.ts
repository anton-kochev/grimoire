...
export function buildEvidencePrompt(
  ...
  lines.push(
    ...
    '1. **[lever]** <one imperative sentence stating the concrete change — what to edit and how, e.g. "Add to the prompt: run single-purpose Bash calls; never chain commands with && or ;">',
    '   - Evidence: <runs + pattern frequency, e.g. "2 shell-syntax errors in Run 1; 4/6 runs re-read the same file"> · Confidence: high|medium|low · Expected effect: <which number above should move, and how>',
    'The first line must be actionable on its own — the reader applies it without reading the evidence. Diagnosis goes in the Evidence line, never before the change. Valid levers: prompt, tools, file_patterns, skills, model. If the agent looks healthy and efficient, say so plainly and keep this section minimal instead of inventing issues.',
    '',
    'Rules:',
